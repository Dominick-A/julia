...
// the slower version in the shared object. The C code in different library
// or executable, however, has to agree on which version to use.
...
// real address directly. However, since the static version detection in `ifunc`
// is not guaranteed to be reliable, we still need to fallback to the wrapper
// version if we didn't find the static version in `ifunc`.
...
#    ifdef JL_TLS_USE_IFUNC
static jl_get_ptls_states_func jl_get_ptls_states_resolver(void)
{
    if (jl_tls_states_cb != jl_get_ptls_states_init)
        return jl_tls_states_cb;
    ...
    // This may not be necessary once
    if (!jl_get_ptls_states_static)
...
JL_DLLEXPORT JL_CONST_FUNC jl_tls_states_t *(jl_get_ptls_states)(void)
    __attribute__((ifunc ("jl_get_ptls_states_resolver")));
